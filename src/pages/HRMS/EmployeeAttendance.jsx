...
const EmployeeAttendance = () => {
  ...
  const handleSelectionCategory = (category)=>{
    if(category==='Faculty'){
       const categoryFilterData = employeeAttendanceData.filter(employee=> employee.designation===category);
       setFilteredEmployeeData(categoryFilterData)
  }else{setFilteredEmployeeData(employeeAttendanceData)}}
  ...
  return (
    <div className="flex flex-col gap-4 w-full overflow-hidden rounded-2xl ">
      <EmployeeAttendanceButtons handleSelectionCategory={handleSelectionCategory}/>
